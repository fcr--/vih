...
data WTManager = WTMa {lo :: Layout -- current layout
                      ,Vty :: vt -- current display manager
...
detachBuffer :: [Int] -> WTManager -> WTManager
detachBuffer (w:ws) -> wTManager
...
getLine wtm stinit = lineInterp wtm stinit 1
lineInterp :: WTManager -> String -> Int -> IO (Maybe String)
lineInterp wtm accstr pos = do
        let vty = Vty.
        {- imprimir imagen -}
        ...
        ev <- next_event vty 
        case ev of 
            (EvResize x y) -> printloop vty wtm x y >> lineInterp wtm accstr pos
            (EvKey key mod) -> case key of
                Kesc -> return Nothing
                KBS -> lineInterp wtm 
                KDel -> lineInterp wtm (if pos = (length accstr) then 
                KLeft -> 
                KRight -> 
                KEnter -> return (Just accstr)
                (KASCII k) -> lineInterp 
